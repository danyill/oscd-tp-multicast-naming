...
* All tripping or testing messages (as defined by control block names beginning `Ctl` or `Trip`, `SPSStn`, `SPSBus` or `Test`) have a `MinTime` of 4 ms and a `MaxTime` of 1000 ms.
* All other messages have a `MinTime` of 100 ms and a `MaxTime` of 1000 ms.
A special case is the GE Multilin B30 relay which has special requirements. 
In this case a `MaxTime` of 2000 ms is used (identified by the IED type and manufacturer attributes).

VLAN ID::
* VLANs are allocated on the basis of the control block name for GSE and the `smvID` for SMV (after the trailing `/`)
* They are allocated as pairs for Prot1 and Prot2 at the same time. 
Prot1 and Prot2 are identified by the IED name ending in an even and odd number respectively.
* A VLAN is either substation wide or on a per primary system bus basis.
* Allocation is as follows, based on the control block name for GOOSE or smvID for SMV traffic.
* For GOOSE control block names the control block name is accepted or the name with an `_` and any other characters.
+
--
.For a Substation
...
|===
|Service Type
|Services (Control Block Name Prefix)
|Prot1 VLAN Range
|Prot2 VLAN Range
|Comment
...
`AdjSwgrPos`
2+|1050-1059
<.^|Passing adjacent switchgear position to Prot 2 devices.
